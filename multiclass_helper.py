...
def plot_MC_boundaries_sklearn(X_train, y_train, score, probability_func, degree=None, bias=False, mesh_res = 300, ax = None, margin=0.5, color_index = 0, normalize = False):
    enc = OneHotEncoder(handle_unknown='ignore')
    y_train_cat_aux = enc.fit_transform(y_train.reshape(-1,1)).toarray()
    ...
    for n, layer_size in enumerate(layer_sizes):
        ...
        for m in range(layer_size):
            ...
            if n == 0:
                ...
                plt.text(n*h_spacing + left+0.10, layer_top - m*v_spacing, r'$\hat{p}_{'+str(m+1)+'}$', fontsize=15)
